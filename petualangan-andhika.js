...
<style>
body{
margin:0;overflow:hidden;width:100vw;height:100vh;position:fixed;top:0;left:0;touch-action:none;font-family:Arial;
}
...
#levelBar{height:100%;width:0%;background:linear-gradient(90deg,#ffe082,#ff9800,#f44336);transition:width .2s linear;}
...
#gitar{position:absolute;font-size:50px;bottom:130px;left:300px;z-index:2;transition:transform .15s ease;}
.bomb{
position:absolute;
width:34px;
height:34px;
border-radius:50%;
z-index:2;
background:radial-gradient(circle at 30% 28%,#616161 0%,#212121 62%,#000 100%);
box-shadow:inset -3px -3px 6px rgba(0,0,0,.45),0 3px 8px rgba(0,0,0,.45);
}
.bombFuse{
position:absolute;
width:14px;
...
.dropShield{animation:floatItem 1.4s ease-in-out infinite;}
.dropFood{animation:floatItem 1.1s ease-in-out infinite;}
...
#gameOverBox{width:min(360px,90vw);background:#fff;border-radius:20px;padding:18px;text-align:center;box-shadow:0 12px 35px rgba(0,0,0,.3);}
...
@media (max-width:480px){#scoreCard{font-size:17px;padding:8px 12px;}#levelWrap{width:120px;}.btn{width:78px;height:78px;font-size:34px;}}
</style>
<div id="gameBg"></div><div id="nightOverlay"></div><div id="starLayer"></div><div id="mountainBack"></div><div id="mountainFront"></div><div id="treeLine"></div><div id="grassFar"></div><div id="grassNear"></div>
...
<div id="hud"><div id="scoreCard"><span id="score">❤️❤️❤️ ⭐ 0</span></div><div id="levelWrap"><div id="levelBar"></div></div><button id="muteBtn" onclick="toggleMute()">🔊</button></div>
<div id="player"><div id="playerGlow"></div><div id="shieldAura"></div></div>
<div id="gitar" style="position:absolute;top:20px;left:300px;font-size:50px;">🎸</div>
<div id="controls"><button class="btn" ontouchstart="kiri()" onclick="kiri()">⬅️</button><button class="btn" ontouchstart="kanan()" onclick="kanan()">➡️</button></div>
<div id="gameOver"><div id="gameOverBox"><h2>💀 Game Over</h2><p id="finalScore">Skor Akhir: 0</p><button id="btnUlang" class="goBtn" onclick="location.reload()">Main Lagi</button><button id="btnQuiz" class="goBtn" onclick="bukaQuizAndhika()">Coba Game Quiz Andhika</button><button id="btnKeluar" class="goBtn" onclick="tutupGameOver()">Lihat Dulu</button></div></div>
`;
...
window.posisi = 150; window.skor = 0; window.nyawa = 3; window.gameSelesai = false; window.muted = false;
...
window.nextHeartAt = 220; window.nextShieldAt = 380; window.nextFoodAt = 120;
...
if(nyawa <= 0){
gameSelesai = true;
document.getElementById("finalScore").innerText = "Skor Akhir: " + skor;
...
invite.innerHTML = '<strong>✨ Coba game Quiz Andhika yuk?</strong><span class="invite-go">Main Quiz</span><button class="invite-close" aria-label="Tutup">×</button>';
...
function updateHud(){
document.getElementById("score").innerText = "❤️".repeat(Math.max(nyawa,0)) + " ⭐ " + skor;
...
bomb.className = "bomb";
let fuse = document.createElement("span");
fuse.className = "bombFuse";
bomb.appendChild(fuse);
document.body.appendChild(bomb);
let startX = ambilPosisiSpawn(90);
return {el:bomb,x:startX,y:-60,speed:6};
}
...
f.className = "dropItem dropFood";
f.innerText = "🍗";
document.body.appendChild(f);
let x = ambilPosisiSpawn(85);
foodList.push({el:f,x:x,y:-60,speed:kecepatan + 1});
...
if(Math.abs((posisi + 50) - (it.x + 20)) < 52 && it.y > 325 && it.y < 490){
it.el.remove();
foodList.splice(i,1);
i--;
skor += 5;
updateHud();
spawnPartikel(posisi + 50, it.y);
continue;
}
if(it.y > 560){
it.el.remove();
...
h.className = "dropItem dropHeart";
h.innerText = "❤️";
...
it.el.style.left = it.x + "px";
it.el.style.top = it.y + "px";
if(Math.abs(posisi - it.x) < 55 && it.y > 320 && it.y < 485){
it.el.remove();
...
s.className = "dropItem dropShield";
s.innerText = "🛡️";
...
let playerLeft = posisi;
let playerTop = window.innerHeight - 220;
let playerRight = playerLeft + 100;
...
if(gameSelesai){return;}
posisi += 20;
let batasKanan = window.innerWidth - 100;
...
if(Math.abs(posisi - gx) < 45 && gy > 350){
skor += 10;
invulnFrames = 8;
mainkanSuara(suaraTangkap);
if(skor % 30 === 0){ mainkanSuara(suaraMantap); }
else if(skor % 20 === 0){ mainkanSuara(suaraBagus); }
else if(skor % 10 === 0){ mainkanSuara(suaraNice); }
yGitar = 20;
gitar.style.top = yGitar + "px";
gitar.style.transform = "scale(1.5)";
updateHud();
spawnPartikel(posisi + 50, gy);
...
for(let i=0;i<bomList.length;i++){
let b = bomList[i];
if(Math.abs(posisi - b.x) < 70 && b.y > 280 && b.y < 500){
b.el.remove();
bomList.splice(i,1);
i--;
}
